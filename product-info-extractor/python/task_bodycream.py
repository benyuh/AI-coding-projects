...
# 2. 当前要处理的【品类名称】 (必须和 images 下的文件夹名字一致)
CATEGORY_NAME = "bodycream"

# 3. 针对该品类的提取要求 (Prompt)
#    如果换了品类（比如卖鞋），可以在这里改提取字段（比如增加“鞋码”、“材质”）
PROMPT_TEXT = """
请分析这张商品详情页图片，提取以下关键信息。
请直接返回纯 JSON 格式数据，不要使用 Markdown 标记。

字段要求：
1. product_name: 商品名称（通常在字号最大的位置）。
2. price: 商品价格（提取数字即可）。
3. selling_points: 卖点（注意：通常位于商品名称的下一行，或者名称附近的描述性短句）。
4. specifications: 规格（如净含量、毫升数、适用肤质等）。
